...
class oscillatorEnv(gym.Env):
  ...
  def __init__(self, len_state=250, ep_length=10000, nosc=1000, epsilon=0.03, frrms=0.1,random=False,
    ...
    self.observation_space = Box(low=-1.5, high=1.5, shape=(2, len_state), dtype=np.float32)
  ...
  def step(self, action):
    ...
    #   return arrayed_version, self.Reward(self.x_val,self.x_state,val), self.done, {} 
      obs = np.stack([self.x_state, self.y_state], axis=0)  # shape: (2, len_state)
      return obs, self.Reward(self.x_val, self.x_state, val), self.done, {}
  ...
  def reset(self):
    ...
    obs = np.stack([self.x_state, self.y_state], axis=0)  # or use np.concatenate for flat
    return obs
